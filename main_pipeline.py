...
def pre_processing(raw, l_freq, h_freq, reference='average', only_language=True):
    ...
    raw.notch_filter(freqs=[50], filter_length='auto', phase='zero', verbose=True)
...
def create_epochs(raw, EPOCHS, subject, session):
    ...
    for condition in ['perception']:
        ...
        for stim in range(1, 31):
            ...
            temp_epoch.resample(250, verbose=True) # 250 -> 128
...
if __name__ == '__main__':
    # General Variables
    DATASET_FOLDER_PATH = 'F:\Intership\eeglab_current\ds004279\SubDataset3'  # Here all the subjects are stored
    ...
    # Step 7. Save data
    final_file = f"Fs250_{l_freq}-{h_freq}hz_20sessions_perception_03.npz"  # Extended: stars one second before the onset of the stimulis
